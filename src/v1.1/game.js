...
const STEP_INFO = {
  evaluate: {
    desc: 'Runs each creature in the physics simulation and measures how far it travels. Fitness = distance from start line. This step is always first and cannot be removed.',
    params: {},
  },
  elitism: {
...
function drawScene() {
  ...
  creatures.forEach((creature, ci) => {
    const hue = (ci / creatures.length) * 60 + 140;
...
function gameLoop(timestamp) {
  ...
  // Camera follows lead creature, ground stays vertically centered
  ...
  if (STATE.simTime >= CONFIG.evalTime) endGeneration();
...
document.getElementById('eval-time').addEventListener('change', (e) => { CONFIG.evalTime = parseInt(e.target.value) || 10; });
