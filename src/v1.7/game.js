...
const BODY_TYPES = {
  roller: {
    name: 'Roller',
    desc: '3 circles linked in a chain. Evolves rolling and inchworm gaits. Simple and effective.',
    segments: 3,
    genomeSize: 9,
    // Preview: array of {x, y, r} for circles and {ax,ay,bx,by} for links
    preview: {
      circles: [{x:20,y:28,r:8},{x:40,y:28,r:10},{x:62,y:28,r:7}],
      links: [[20,28,40,28],[40,28,62,28]],
    },
  ...
  hopper: {
    name: 'Hopper',
    desc: 'Heavy body with a spring-loaded leg. Bounces and hops. Needs high frequency oscillation.',
    segments: 2,
    genomeSize: 8,
    preview: {
      circles: [{x:40,y:18,r:12},{x:40,y:44,r:6}],
      links: [[40,18,40,44]],
    },
    build(decoded, startX, startY, index) {
      const bodies = [];
  ...
  snake: {
    name: 'Snake',
    desc: '5-segment undulating chain. Low friction, many joints. Evolves worm-like and serpentine movement.',
    segments: 5,
    genomeSize: 17,
    preview: {
      circles: [{x:10,y:28,r:5},{x:24,y:28,r:5},{x:38,y:28,r:6},{x:52,y:28,r:5},{x:66,y:28,r:5}],
      links: [[10,28,24,28],[24,28,38,28],[38,28,52,28],[52,28,66,28]],
    },
    build(decoded, startX, startY, index) {
      const bodies = [];
      const constraints = [];
  ...
  bipod: {
    name: 'Bipod',
    desc: 'Central body with 2 independent legs. Can evolve walking, galloping, or dragging gaits.',
    segments: 3,
    genomeSize: 13,
    preview: {
      circles: [{x:40,y:18,r:10},{x:24,y:44,r:5},{x:56,y:44,r:5}],
      links: [[40,18,24,44],[40,18,56,44]],
    },
    build(decoded, startX, startY, index) {
      const bodies = [];
      const constraints = [];
  ...
  triball: {
    name: 'Triball',
    desc: 'Triangle of 3 balls, all connected. Tumbles and rolls chaotically. Surprisingly effective.',
    segments: 3,
    genomeSize: 15,
    preview: {
      circles: [{x:40,y:12,r:8},{x:22,y:40,r:7},{x:58,y:40,r:9}],
      links: [[40,12,22,40],[22,40,58,40],[58,40,40,12]],
    },
    build(decoded, startX, startY, index) {
      const bodies = [];
      const constraints = [];
...
const STEP_INFO = {
  evaluate: {
    desc: 'Runs each creature in the physics simulation and scores them based on the chosen fitness mode.',
    params: {
      fitnessMode: null, // special: rendered as separate block
      timeLimit: 'Seconds each generation runs before stopping. Longer = more time to travel, but slower generations.',
      earlyStop: 'Ends the generation early when all creatures have stopped, finished, or died. Saves time.',
    },
    fitnessModes: {
      distance: { name: 'Distance', desc: 'Fitness = how far right the creature gets. Simple and reliable. Evolution always has a gradient to follow since any movement to the right is rewarded.' },
      finish_speed: { name: 'Finish Speed', desc: 'Fitness = distance + large time bonus for crossing the finish line. Creatures that finish get scored by speed. Pushes evolution past "good enough" distance toward actually completing the course fast.' },
      efficiency: { name: 'Efficiency', desc: 'Fitness = distance minus energy spent. Penalizes creatures that flail wildly. Evolves smooth, economical locomotion. May produce slower but more elegant movers.' },
      sprint: { name: 'Sprint', desc: 'Only finishers get real scores (ranked by finish time). Non-finishers get just 10% of their distance. Very hard — evolution must first solve distance, then optimize speed. Works best with longer time limits.' },
    },
  },
  elitism: {
...
// ============================================================
let pipeline = [
  { type: 'evaluate', removable: false, config: { timeLimit: 15, earlyStop: true, fitnessMode: 'distance' } },
...
function applyOscillation(creature, time) {
  ...
  for (let i = 0; i < joints.length; i++) {
    ...
    // Track energy for efficiency mode
    creature._energyUsed = (creature._energyUsed || 0) + Math.abs(force) + Math.abs(axialForce);
  }
}
...
function evaluateFitness(creature) {
  if (creature.dead) return creature.fitness;
  ...
  const mode = evalStep?.config?.fitnessMode || 'distance';
  const timeLimit = evalStep?.config?.timeLimit || CONFIG.evalTime;
  const dist = Math.max(0, creature.maxX - CONFIG.startX);

  switch (mode) {
    case 'distance':
      creature.fitness = dist;
      break;
    ...
    case 'finish_speed':
      ...
      if (creature.finished && creature.finishTime != null) {
        ...
        creature.fitness = (CONFIG.finishX - CONFIG.startX) + timeBonus;
      }
      break;

    case 'efficiency':
      // Distance minus "energy" — penalize high total force usage
      creature._energyUsed = (creature._energyUsed || 0);
      creature.fitness = Math.max(0, dist - creature._energyUsed * 0.1);
      break;

    case 'sprint':
      // Only finishers get scored, ranked by speed
      if (creature.finished && creature.finishTime != null) {
        creature.fitness = (CONFIG.finishX - CONFIG.startX) + (timeLimit - creature.finishTime) * 200;
      } else {
        // Non-finishers get low fitness proportional to distance (so evolution has gradient)
        creature.fitness = dist * 0.1;
      }
      break;
...
function worldToScreen(wx, wy) {
  ...
  // Fixed scale: 1 world unit = 0.2 screen pixels at zoom=1
  // This means creatures stay the same apparent size regardless of track length
  // Short track = more empty space, long track = need to pan
  const baseScale = 0.2;
...
function drawScene() {
  ...
  if (STATE.ghostReplay && STATE.ghostReplay.length > 0) {
    ...
    if (frame) {
      ...
      frame.forEach(pos => {
        ...
        ctx.beginPath();
        ctx.arc(sp.x, sp.y, pos.r * getScale(), 0, Math.PI * 2);
        ctx.fill();
      });
      ctx.globalAlpha = 1;
    }
  ...
  drawCreatures.forEach((creature, ci) => {
    ...
    creature.bodies.forEach((body, bi) => {
      ...
      const r = creature.decoded.sizes[bi] * getScale();
...
function showTooltip(stepType, anchorEl) {
  ...
  // Fitness modes (special section for evaluate)
  if (info.fitnessModes) {
    html += '<div class="info-tooltip-params">';
    html += '<div class="info-tooltip-param" style="margin-bottom:4px"><span class="info-tooltip-param-name">Fitness Modes</span></div>';
    for (const [key, mode] of Object.entries(info.fitnessModes)) {
      html += `<div class="info-tooltip-param" style="margin-bottom:4px"><span class="info-tooltip-param-name">${mode.name}:</span> ${mode.desc}</div>`;
    }
    html += '</div>';
  }

  const paramKeys = Object.keys(info.params).filter(k => info.params[k] !== null);
...
function renderPipeline() {
  ...
  pipeline.forEach((step, idx) => {
    ...
    switch (step.type) {
      case 'evaluate':
        configDiv.appendChild(makeSelectInput('Fitness', step.config, 'fitnessMode',
          ['distance', 'finish_speed', 'efficiency', 'sprint'],
          ['Distance', 'Finish Speed', 'Efficiency', 'Sprint']
        ));
...
function makeNumberInput(label, configObj, key, min, max, step) {
  ...
  input.addEventListener('change', () => { configObj[key] = parseFloat(input.value); scheduleSave(); });
  row.appendChild(lbl); row.appendChild(input);
  return row;
}

function makeSelectInput(label, configObj, key, options, labels) {
  const row = document.createElement('div');
  row.className = 'config-row';
  const lbl = document.createElement('span');
  lbl.className = 'config-label';
  ...
  select.addEventListener('change', () => { configObj[key] = select.value; scheduleSave(); renderPipeline(); });
...
function makeCheckboxInput(label, configObj, key) {
  ...
  cb.addEventListener('change', () => { configObj[key] = cb.checked; scheduleSave(); });
...
function drawBodyPreview() {
  const pCanvas = document.getElementById('body-preview');
  ...
  // Links
  pCtx.strokeStyle = '#5b8aff60';
  pCtx.lineWidth = 2;
  preview.links.forEach(([ax, ay, bx, by]) => {
    pCtx.beginPath();
    pCtx.moveTo(ax, ay);
    pCtx.lineTo(bx, by);
    pCtx.stroke();
  ...
  // Circles
  preview.circles.forEach(c => {
    pCtx.fillStyle = '#00e4a080';
    pCtx.strokeStyle = '#00e4a0c0';
    pCtx.lineWidth = 1.5;
    pCtx.beginPath();
  ...
  document.getElementById('body-desc').textContent = bt.desc || '';
}
...
document.getElementById('pop-size').addEventListener('change', (e) => { CONFIG.popSize = parseInt(e.target.value) || 30; scheduleSave(); });

document.getElementById('track-length').addEventListener('change', (e) => {
  CONFIG.finishX = parseInt(e.target.value);
  scheduleSave();
  // Full reset with new track
  STATE.generation = 0;
  STATE.bestAllTime = null;
  STATE.fitnessHistory = [];
  STATE.ghostReplay = null;
...
// ============================================================
const STORAGE_KEY = 'evolab_config';
...
function saveConfig() {
  try {
    const data = {
      ...
      pipeline: pipeline.map(s => ({ type: s.type, config: s.config ? { ...s.config } : undefined })),
    };
...
function loadConfig() {
  try {
    ...
    if (data.popSize) CONFIG.popSize = data.popSize;
    if (data.bodyType) CONFIG.bodyType = data.bodyType;
    if (data.finishX) CONFIG.finishX = data.finishX;
    if (data.speedMultiplier) CONFIG.speedMultiplier = data.speedMultiplier;
    if (data.displayMode) CONFIG.displayMode = data.displayMode;

    // Restore pipeline configs
    if (data.pipeline && Array.isArray(data.pipeline)) {
      data.pipeline.forEach(saved => {
        const step = pipeline.find(s => s.type === saved.type);
        if (step && saved.config) {
          Object.assign(step.config, saved.config);
        }
      });
    }

    // Sync UI elements
    document.getElementById('pop-size').value = CONFIG.popSize;
    document.getElementById('body-type').value = CONFIG.bodyType;
    document.getElementById('track-length').value = CONFIG.finishX;

    // Speed buttons
    document.querySelectorAll('.speed-btn').forEach(b => {
      b.classList.toggle('active', parseInt(b.dataset.speed) === CONFIG.speedMultiplier);
    });

    // Display mode button
    const dispBtn = document.getElementById('btn-display');
    if (CONFIG.displayMode === 'best') {
      dispBtn.textContent = '👁 Best';
...
// Wrap config setters to trigger save
const origConfigProxy = new Proxy(CONFIG, {
...
// We can't easily proxy CONFIG itself since it's already referenced everywhere,
// so we'll call scheduleSave() explicitly from event handlers.
