...
const STEP_INFO = {
  evaluate: {
    desc: 'Runs each creature in the physics simulation and scores them. Choose a fitness mode to control what evolution optimizes for.',
    params: {
      fitnessMode: 'Distance: pure horizontal distance from start. Finish Speed: distance + big bonus for crossing the finish line quickly. Efficiency: distance penalized by energy used (favors smooth movers). Sprint: only creatures that finish are scored, ranked by time.',
      timeLimit: 'Seconds each generation runs before stopping. Longer = more time for creatures to travel, but slower generations.',
      earlyStop: 'If enabled, ends the generation early when all creatures have stopped, finished, or died.',
    },
  },
  elitism: {
