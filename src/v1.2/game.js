...
const CONFIG = {
  ...
  creatureSegments: 3,
  genomeSize: null,
  groundY: 400,         // fixed world-space Y (not canvas-relative)
  startX: 80,
  finishX: 3000,
  gravity: { x: 0, y: 1.2 },
  speedMultiplier: 1,
...
function decodeGenome(genome) {
  ...
  for (let i = 0; i < nSeg; i++) {
    sizes.push(8 + Math.abs(genome[i]) * 12);
  }
...
function evaluateFitness(creature) {
  ...
  if (evalStep?.config?.finishBonus && !creature.finished && creature.maxX >= CONFIG.finishX) {
    creature.finished = true;
    creature.finishTime = STATE.simTime;
    // Bonus: base distance + time bonus (faster = more bonus)
    const timeLimit = evalStep.config.timeLimit || CONFIG.evalTime;
    const timeBonus = Math.max(0, timeLimit - STATE.simTime) * 100;
    creature.fitness += timeBonus;
  }
...
function worldToScreen(wx, wy) {
  ...
  const sy = (wy - STATE.camera.y) * STATE.camera.zoom + ch * 0.55;
  return { x: sx, y: sy };
}
...
function drawScene() {
  ...
  drawCreatures.forEach((creature, ci) => {
    ...
    const hue = isBest ? 40 : (ci / creatures.length) * 60 + 140;
    const alpha = isBest ? 0.95 : 0.7;
...
function gameLoop(timestamp) {
  ...
  // Camera: follow based on display mode
  let leadX = CONFIG.startX;
  if (CONFIG.displayMode === 'best') {
    let bestCreature = creatures[0];
    creatures.forEach(c => { if (c.fitness > bestCreature.fitness) bestCreature = c; });
    if (bestCreature) leadX = bestCreature.maxX;
  } else {
    creatures.forEach(c => { if (c.maxX > leadX) leadX = c.maxX; });
  }
  STATE.camera.x += (leadX - 200 - STATE.camera.x) * 0.03;
  STATE.camera.y += (CONFIG.groundY - STATE.camera.y) * 0.05;
  ...
  // Early stop: if all creatures stalled or finished
  if (!shouldEnd && earlyStop && STATE.simTime > 2) {
    const allDone = creatures.every(c => c.finished || isCreatureStalled(c));
    if (allDone) shouldEnd = true;
  }

  // All finished
  if (!shouldEnd) {
    const allFinished = creatures.every(c => c.finished);
    if (allFinished) shouldEnd = true;
  }
...
function renderPipeline() {
  ...
  pipeline.forEach((step, idx) => {
    ...
    if (step.removable !== false && step.type !== 'evaluate') {
      ...
      removeBtn.onclick = (e) => { e.stopPropagation(); pipeline.splice(idx, 1); renderPipeline(); };
    ...
    if (step.type !== 'evaluate') {
      node.addEventListener('dragstart', (e) => { e.dataTransfer.setData('text/plain', idx); node.classList.add('dragging'); });
      node.addEventListener('dragend', () => node.classList.remove('dragging'));
      node.addEventListener('dragover', (e) => e.preventDefault());
      node.addEventListener('drop', (e) => {
        e.preventDefault();
        const fromIdx = parseInt(e.dataTransfer.getData('text/plain'));
        if (fromIdx !== idx && fromIdx > 0 && idx > 0) {
...
document.getElementById('btn-reset').addEventListener('click', () => {
  ...
  STATE.camera = { x: 0, y: CONFIG.groundY, zoom: 1 };
